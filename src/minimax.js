...
export async function preprocessMusicCover(referenceAudio) {
  ...
  const rawStructure = json?.structure_result || json?.data?.structure_result || '';
  let structure = [];
  try {
    const parsed = typeof rawStructure === 'string' ? JSON.parse(rawStructure) : rawStructure;
    if (Array.isArray(parsed?.segments)) {
      structure = parsed.segments
        .map((segment) => ({ start: Number(segment.start), end: Number(segment.end), label: String(segment.label || 'section') }))
        .filter((segment) => Number.isFinite(segment.start) && Number.isFinite(segment.end) && segment.end > segment.start);
    }
  } catch { /* 结构化歌词仍可用，结构信息仅用于 UI 辅助展示。 */ }
  const duration = Number(json?.audio_duration || json?.data?.audio_duration || 0);
  return { coverFeatureId, lyrics: json?.formatted_lyrics || json?.data?.formatted_lyrics || '', structure, duration: Number.isFinite(duration) ? duration : 0 };
}
